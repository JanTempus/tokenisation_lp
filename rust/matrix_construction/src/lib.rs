...
impl TokenInstance {
    #[new]
    pub fn new(start: usize, end: usize, token: String, lp_value: f64) -> Self {
        TokenInstance { start, end, token, lp_value }
    }
}
...
#[pymodule]
fn matrix_construction(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_class::<TokenInstance>()?;
    m.add_class::<PossibleToken>()?;
    m.add_function(wrap_pyfunction!(make_matrix, m)?)?;
    m.add_function(wrap_pyfunction!(make_matrix_parallel, m)?)?;
    Ok(())
}
